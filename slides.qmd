...
Chamamos o termo $R_0 = (\beta / \gamma) S(0)$ de
...
def graficoSIRV(mi):
  y0 = [S0, I0, R0, V0] # condicoes iniciais
  ...
  plt.plot(t_sol, y_sol[:,1], 'r', label='Infectados (I)')
  plt.plot(t_sol, y_sol[:,2], 'b', label='Removidos (R)')
  plt.plot(t_sol, ysol[:,3], 'purple', label='Vacinados (V)')
  plt.xlabel('Tempo (dias)')
  plt.ylabel(r'Nº de indivíduos ($\times 10^4$)')
...
Neste caso, o número de vacinados $V$ se torna constante: $V = \rho S(0) \tag{8}$. Logo, o número de indivíduos suscetíveis no início da epidemia já não é mais o total, e sim $(1 - \rho) S(0)$.
