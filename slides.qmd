...
- $\mathrm{\textcolor{ForestGreen}{Suscetíveis}}$ à contaminação: nunca apresentaram a doença, mas podem contrair;
- $\mathrm{\textcolor{red}{Infectados}}$: atualmente infectados com a doença;
- $\mathrm{\textcolor{blue}{Removidos}}$: foram infectados e ganharam imunidade permanente.
...
- $\mathrm{\textcolor{purple}{Vacinados}}$: Foram vacinados contra a doença e ganharam imunidade permanente.
...
def rk4_sys_SIRV(f, ti, tf, h, y0, ni):
  ...
  for i in range(n_steps):
      t = tpoints[i]
      k1 = h * fSIRV(y, t, ni)
      k2 = h * fSIRV(y + 0.5 * k1, t + 0.5 * h, ni)
      k3 = h * fSIRV(y + 0.5 * k2, t + 0.5 * h, ni)
      k4 = h * fSIRV(y + k3, t + h, ni)
      y = y + (1.0/6.0) * (k1 + 2*k2 + 2*k3 + k4)
      ypoints[i+1] = y
...
```{python}
#| fig-align: center
#| fig-cap: Simulação do modelo SIRV
